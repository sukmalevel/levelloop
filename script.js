...
// === DOM Elements (ambil setelah DOM siap) ===
let dropZone, fileInput, video, controls, setLoopBtn, clearLoopBtn;
let startTimeInput, endTimeInput, exportVideoBtn, aiSuggestBtn;
let codeModal, accessCodeInput, cancelCodeBtn, submitCodeBtn;
let loading;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setLoopBtn = document.getElementById('set-loop');
  clearLoopBtn = document.getElementById('clear-loop');
  startTimeInput = document.getElementById('start-time');
  endTimeInput = document.getElementById('end-time');
  exportVideoBtn = document.getElementById('export-video');
  aiSuggestBtn = document.getElementById('ai-suggest');

  codeModal = document.getElementById('code-modal');
  accessCodeInput = document.getElementById('access-code');
  cancelCodeBtn = document.getElementById('cancel-code');
...
function initApp() {
  ...
  async function downloadLoopedClip(filename) {
    try {
      ...
      // 🔽 Optimasi untuk HP: ultrafast + resolusi kecil
      await ffmpeg.run(
        "-i", "input.mp4",
